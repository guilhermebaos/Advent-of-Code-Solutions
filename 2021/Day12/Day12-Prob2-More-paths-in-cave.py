...
def search_all_paths(connections: dict, path: list, small_cave_twice=False):
    global all_paths
    check_next = path[-1]
    ...
    for cave in connections[check_next]:
        ...
        # Only explore each small cave once, except one small cave which we can explore twice
        elif cave.islower():
            if cave not in path:
                search_all_paths(connections, path + [cave], small_cave_twice)
            else:

                # Explore a small cave twice if we haven't already
                if not small_cave_twice:
                    search_all_paths(connections, path + [cave], True)
