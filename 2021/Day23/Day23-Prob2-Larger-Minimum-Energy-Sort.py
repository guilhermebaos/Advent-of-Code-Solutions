...
# Add to the map the two missing lines
def add_missing_lines(raw_burrow: list):
    missing = ['  #D#C#B#A#  ', '  #D#B#A#C#  ']
    for line in missing:
        raw_burrow.insert(len(raw_burrow) - 2, line)
    return raw_burrow
...
def possible_moves(coords, hallway_coords: list, room_coords: list):
    ...
    for start in hallway_coords:
        ...
        if room_right_amphipods(coords, amphipod):
            ...
            for end_y in range(3, -1, -1):
                if coords[amphipod + str(end_y)] == '.':
                    end = amphipod + str(end_y)
                    break
...
def mini_energy_sort(burrow: list):
    burrow = add_missing_lines(burrow)
    ...
    solution = {'H0': '.', 'H1': '.', 'H2': '.', 'H3': '.', 'H4': '.', 'H5': '.', 'H6': '.', 'H7': '.', 'H8': '.',
                'H9': '.', 'Ha': '.', 'A0': 'A', 'A1': 'A', 'A2': 'A', 'A3': 'A', 'B0': 'B', 'B1': 'B', 'B2': 'B',
                'B3': 'B', 'C0': 'C', 'C1': 'C', 'C2': 'C', 'C3': 'C', 'D0': 'D', 'D1': 'D', 'D2': 'D', 'D3': 'D'}
